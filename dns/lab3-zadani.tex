...
\begin{itemize}
  ...
  \item Pracujte ve dvojicích, zkontrolujte, že máte počítač propojen
    přímým kabelem s přístupovým přepínačem (rozhraní eth0, zdířka E na patch panelu).
\end{itemize}
...
\begin{enumerate}
  ...
  \item Nastavte statické IP adresy na rozhraní eth1 vašich počítačů pomocí {\tt 
  ...
  \item Na PC A rozšiřte konfiguraci DNS serveru tak, aby obsluhoval požadavky pro Vámi
    ...
    Needitujte však soubor přímo, ale zkopírujte jej do {\tt /var/named/bind}, 
    nezapomeňte zaregistrovat zóny v~souboru \linebreak{\tt named.conf.local} 
  ...
  \item V konfiguračním souboru {\tt hcpd} nastavte na obou počítačích použití vlastního
